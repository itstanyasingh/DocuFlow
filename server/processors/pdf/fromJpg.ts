...
import { validateJpg } from './validation';

export interface ImageToPdfOptions {
  pageSize?: 'original' | 'a4' | 'a5' | 'letter' | 'legal';
...
/**
 * Convert multiple JPG/JPEG images into a single structured PDF document
 */
export async function convertJpgToPdf(
  ...
  for (const img of images) {
    const val = validateJpg(img.buffer, img.fileName);
    if (!val.isValid) {
      throw new Error(`File "${img.fileName}" is not a valid JPG: ${val.error}`);
    }
  }
  ...
  for (const img of images) {
    const embedded = await pdfDoc.embedJpg(img.buffer);
